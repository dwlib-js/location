...
const ToDecimal = require('#type/ToDecimal');
const ToString = require('#type/ToString');
...
const LocationEquals = (location, other) => {
  const latitude = RequireInternalSlot(location, $Latitude);
  const longitude = GetInternalSlot(location, $Longitude);
  if (location === other) {
    return true;
  ...
  const otherLatitude = GetInternalSlot(other, $Latitude);
  const otherLongitude = GetInternalSlot(other, $Longitude);
  return latitude === otherLatitude && longitude === otherLongitude;
}
...
class Location {
  ...
  equals(other) {
    return LocationEquals(this, other);
  }
